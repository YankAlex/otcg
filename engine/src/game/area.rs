...
impl PlayerArea {
    pub fn new(background: PlayerBackground, rules: &Rules) -> Self {
        Self {
            ...
            special_zone: Arc::new(Pile::new_with_cards(background.special_zone_clone(), false, Visibility::Public, rules.only_raw_card_in_special_zone, background.player)),
        }
    }
}
...
impl FightArea {
    pub fn new(location_cards: Arc<Pile>, players: Vec<Player>) -> Self {
        ...
        sides.append(&mut players.into_iter().map(|pl| Arc::new(Pile::new_empty(false, Visibility::Public, false, pl))).collect::<Vec<_>>());
        Self {
            sides,
        }
    }
